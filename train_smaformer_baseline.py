...
def load_checkpoint(path, model, optimizer, scheduler, scaler, device):
    ...
    # weights_only=False: required because we store RNG state (np.random.get_state()
    # returns a tuple containing a raw ndarray) inside the checkpoint. PyTorch >=2.6
    # defaults torch.load to weights_only=True, which restricts unpickling to a
    # tensor/primitive safelist and rejects numpy's reconstruction internals. This is
    # safe here specifically because these checkpoints are self-generated by this
    # script, not loaded from an untrusted/third-party source -- do not relax this
    # for checkpoints of unknown provenance.
...
def main():
    ...
    # Total epoch budget is anchored to the ORIGINALLY requested start epoch
    # (args.start_epoch), not wherever a given launch happens to resume from.
    # Using `start_epoch + args.epochs` here would re-add a full args.epochs
    # on every resume (e.g. resuming at epoch 16 would run up to epoch 45
    # instead of stopping at the intended 30) -- and since CosineAnnealingLR's
    # T_max is fixed at args.epochs, stepping the scheduler past T_max drives
    # the cosine term past pi, bouncing the LR back up instead of annealing
    # down to eta_min.
    for epoch in range(start_epoch, args.start_epoch + args.epochs):
